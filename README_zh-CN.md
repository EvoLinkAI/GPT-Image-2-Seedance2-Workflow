...
欢迎来到 GPT Image 2 × Seedance 2.0 工作流仓库！🤗

**我们收集了经过验证的工作流、提示词模板，以及创作者的真实案例，帮助你把 GPT Image 2 和 Seedance 2.0 结合起来，制作高质量 AI 视频。**

GPT Image 2 负责“画什么”和视觉一致性，Seedance 2.0 负责“怎么动”，把这些图像真正动画化为视频。两者结合后，构成了当前能力非常强的一条 AI 视频生产流程。

这个仓库中的大多数案例都整理自 X/Twitter 创作者、社区实验以及真实生产工作流。

在 Evolink 上试试：[GPT Image 2 + Seedance 2.0](https://evolink.ai?utm_source=github&utm_medium=readme&utm_campaign=gptimage2-x-seedance2)
...
- **2026 年 4 月 25 日：** 新增案例 10–12（多帧分镜、日系 MV 工具链、Claude Code × 角色设定图），扩展案例 9 的 ARPG 模拟变体，画廊新增社区展示内容
- **2026 年 4 月 23 日：** 仓库发布，首批收录 9 个精选工作流案例
...
- [🎥 分镜工作流](#-分镜工作流)
  ...
  - [案例 2：3×3 网格分镜法（by @servasyy_ai）](#案例-23×3-网格分镜法by-servasyy_ai)
...
- [🎵 创意组合](#-创意组合)
  ...
  - [案例 9：游戏与交互内容（by @AbleGPT）](#案例-9游戏与交互内容by-ablegpt)
...
1. 让 GPT Image 2 把所有分镜一起输出为单张 3×3 网格图
2. 将整张网格图作为一张图片导入 Seedance 2.0
3. Seedance 会把这张网格图读取为动作序列，并生成连续视频
...
1. 用 GPT Image 2 生成角色三视图（正面 / 侧面 / 背面）
2. 如有需要，单独生成装备设定图，再合成为完整装备版三视图
3. 以三视图为参考生成分镜画面
4. 将分镜导入 Seedance 2.0 生成动画
...
1. 用 GPT Image 2 生成一张细致的场景设定图
2. 导入 Seedance 2.0，并配上简短的动作提示词
3. 可选地做对比：一版使用严格分镜控制，一版让 Seedance 自由动画化
...
> [!NOTE]
> 当 Seedance 在没有分镜参考的情况下自由生成时，结果通常会更有动感，但与原图的一致性会下降。关键角色镜头更适合用分镜控制，动作场景则可以尝试自由动画。
...
先用 Claude Code 撰写世界观设定和角色背景，再把结构化描述传给 GPT Image 2 生成角色关键视觉，最后用 Seedance 2.0 制作动画。对开发者友好的原创 IP 创作工作流。191 点赞 / 7K 播放。

<table><tr>
<td align="center"><a href="https://evolink.ai/seedance2?utm_source=github&utm_medium=picture&utm_campaign=gptimage2-x-seedance2"><img src="images/character_case12/output.jpg" width="400" alt="Claude Code + GPT Image 2 角色关键视觉"></a></td>
</tr></table>

**步骤：**

1. 用 Claude Code 起草世界观笔记和结构化角色设定（姓名、外貌、性格、背景）
2. 将角色设定直接传入 GPT Image 2，生成关键视觉或角色设定图
3. 以关键视觉作为参考图导入 Seedance 2.0，进行动画化

**Claude Code → GPT Image 2 交接提示词：**
...
> [!NOTE]
> Claude Code 输出的是结构化文本——角色设定、场景描述、对话大纲——这些作为详细提示词传入 GPT Image 2 效果很好。这条流程线对原创故事 IP 尤其有效：在代码中构建世界观，在 GPT Image 2 中将其可视化，在 Seedance 中做成动画。
...
先用 GPT Image 2 生成一款尚未真正存在的 App 的完成态 UI 截图，再交给 Seedance 2.0 做成产品演示视频。你可以把它发布到 TikTok 或社交平台，在正式开发前先测试市场兴趣。
...
2. 生成 3 到 5 张关键 UI 截图（首页、功能页、个人页等）
...
> [!NOTE]
> **使用前请替换方括号中的占位内容。** 视频文案里不要强调它是 AI 生成的，直接当作产品演示发布，再观察评论区里的真实用户反馈。
...
1. 用 GPT Image 2 生成主视觉（产品 / 角色 / 场景）
2. 基于主视觉生成 4 到 5 格分镜
3. 在 Seedance 2.0 中分别为每格做动画，每段控制在 3 到 4 秒
4. 拼接所有片段，并加入字幕与音乐
...
1. 先在 Suno 中生成目标风格的音乐，并确认歌曲结构（前奏 / 主歌 / 副歌）
2. 在 GPT Image 2 中按照歌曲段落设计分镜
3. 在 Seedance 2.0 中分别做动画，并让片段时长贴合节拍
4. 在剪辑软件中把视频片段与音乐轨道同步
...
> [!NOTE]
> 先做音乐。只有先知道节奏和时长，后面的分镜设计才能精准匹配鼓点切换。
...
1. 在 GPT Image 2 中先定义视觉风格系统，固定颜色、光线和角色形象
2. 生成 4 到 6 张保持相同氛围的图像
3. 在 Seedance 2.0 中为每张图加入缓慢、氛围感强的动作提示
4. 把这些片段串联起来，形成一个简短的视觉叙事
...
### 案例 9：[游戏与交互内容](https://x.com/AbleGPT/status/2047149644778746020)（by [@AbleGPT](https://x.com/AbleGPT)）
...
1. 用 GPT Image 2 生成 ARPG 或游戏 UI 风格图像，包含 HUD 元素
2. 导入 Seedance 2.0，并描述交互过程或战斗流程
3. 再加入后期效果（粒子、辉光）做最终润色
...
**Seedance 2.0：** 使用图生视频模式。无需提示词——Seedance 会自动读取 HUD 布局并将其延伸为游戏画面。

> [!NOTE]
> Seedance 2.0 对写实真人内容存在限制。游戏、动漫和插画风格可以绕开其中大部分限制，也能提供更大的创作空间。
>
> **ARPG 技巧（via [@peter6759](https://x.com/peter6759/status/2047130834180903166)）：** 如需互动电影游戏风格，可以在一步中合并两个步骤——GPT Image 2 提示词：`互动电影游戏，黑神话风格，水浒传` → Seedance 2.0 提示词：`Click option A, normal UI shift, then reasonable combat happens`。中文提示词用于 GPT Image 2、英文用于 Seedance 的双语方式，往往能提升文化还原度。
>
> **社区展示：** [@markgadala](https://x.com/markgadala/status/2047825115631518115) 用这套工作流为一款并不存在的游戏生成了完整预告片。[@0xInk_](https://x.com/0xInk_/status/2047648944004755679) 将其用于高细节 UI 动画（972 点赞 / 7.5 万播放）。
...
1. 先在 Suno 5.5 生成音乐——确定歌曲时长、节奏和情绪
2. 在 GPT Image 2 中按歌曲段落设计分镜
3. 在 Seedance 2.0 中为每格做动画，让片段时长贴合节拍
4. 将视频片段和 Suno 音轨导入 CapCut——同步后导出
...
> [!NOTE]
> 先做音乐——在设计分镜之前先了解节拍结构，才能精准把控每格画面与歌曲切换点的对应关系。这是对案例 7（城市流行 MV）的延伸，将 Suno 纳入流程，并把整个制作视为同步协调的生产，而非后期拼接。

## 💡 技巧与方法

### 一致性指南

在 GPT Image 2 的输出之间，以及进入 Seedance 2.0 动画之后，如何保持视觉一致性，是最常见的难点。下面这些方法分别对应不同层级的问题。

**产品图一致性**

产品在 Seedance 中变形的根本原因是：它的运动插值过程会重写细节，导致 Logo、材质和表面纹理被改动。

解决方法：
- 在 Seedance 提示词中加入 `keep the product appearance completely unchanged, camera movement only, no rotation`
- 优先选择相机运动（推进、拉远），而不是主体本身运动，让产品保持静止，由镜头来动
- 把单段视频控制在 3 秒以内，片段越短，累计失真通常越少

**角色一致性**

- 先生成角色三视图，把它作为后续所有分镜的视觉锚点
- 在每个分镜提示词中都重复一遍简短的角色描述（发色、服装、体型）
- 避免在同一段片段中频繁切换角色视角

**场景一致性**

当你在 GPT Image 2 中生成多张分镜时，可以在提示词顶部先固定场景参数：
...
原因：图片中包含被判定为敏感的内容，比如写实暴力、某些姿态下的人脸。
解决方法：改成动漫或插画风格，或者从提示词中移除人物相关描述。

**输出动作很混乱**

原因：分镜图过于复杂，Seedance 无法判断主要动作方向。
解决方法：把分镜简化成一个主体加一个明确动作，减少背景元素。

**产品图变形**

参考上方“一致性指南 → 产品图一致性”部分。

**平台输入格式要求**

| 平台 | 推荐输入尺寸 | 支持格式 | 最大文件大小 |
| :---: | :---: | :---: | :---: |
...
Evolink 让你可以在一个地方同时使用 GPT Image 2 和 Seedance 2.0，不需要反复切换平台，也不需要重复上传文件。

**为什么用 Evolink**

- GPT Image 2 和 Seedance 2.0 共用一个 API Key
- 同一界面内直接完成图生视频流转，生成图片后点击 “Send to Video” 即可，无需下载再上传
- 支持批量处理，可以把多个分镜依次排队生成视频
...
这个仓库受到了许多优秀开放工作流合集以及社区公开实验的启发。

感谢所有公开分享作品的创作者和贡献者，正是他们的内容让这些案例整理成为可能。
...
*我们无法保证每个案例都已经准确归属到最初创作者。如果有需要修正的地方，请联系我们，我们会及时更新。*

如果你也有更有趣的工作流案例想分享，欢迎联系，一起扩展 Evolink 的工作流案例库。
